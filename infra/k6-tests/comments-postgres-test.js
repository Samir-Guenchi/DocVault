/**
 * K6 Load Test for Comments Service (PostgreSQL Backend)
 * Lab 8 Assignment: Performance Comparison
 * 
 * Run: k6 run --env BASE_URL=http://localhost:8082 comments-postgres-test.js
 */
...
export function handleSummary(data) {
  return {
    'postgres-results.json': JSON.stringify(data, null, 2),
    'postgres-results.txt': textSummary(data, { indent: ' ', enableColors: false }),
...
function textSummary(data, options) {
  ...
  let summary = '\n';
  summary += `${indent}========================================\n`;
  summary += `${indent}  PostgreSQL Backend - Load Test Results\n`;
  summary += `${indent}========================================\n\n`;
